    ...
    if($valid == 1) {
    	...
    	if( isset($_FILES['photo']["name"]) && isset($_FILES['photo']["tmp_name"]) )
        {
            ...
            if(isset($final_name1)) {
		        ...
		        {
		        	$statement = $pdo->prepare("INSERT INTO tbl_product_photo (photo,id) VALUES (?,?)");
        ...
        if(isset($_POST['size'])) {
        	...
        	$statement = $pdo->prepare("DELETE FROM tbl_product_size WHERE id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['size'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_size (size_id,id) VALUES (?,?)");
		...
		if(isset($_POST['color'])) {
			...
			$statement = $pdo->prepare("DELETE FROM tbl_product_color WHERE id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['color'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_color (color_id,id) VALUES (?,?)");
				$statement->execute(array($value,$_REQUEST['id']));
			}
		} else {
			$statement = $pdo->prepare("DELETE FROM tbl_product_color WHERE id=?");
			                        ...
			                        <tbody>
			                        	...
			                        	foreach ($result as $row) {
											...
											<tr>
				                                ...
				                                <td style="width:28px;">
				                                	<a onclick="return confirmDelete();" href="product-other-photo-delete.php?id=<?php echo $row['p_id']; ?>&id1=<?php echo $_REQUEST['id']; ?>" class="btn btn-danger btn-xs">X</a>
				                                </td>
				                            </tr>
			                        		<?php
			                        	}
