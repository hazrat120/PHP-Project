...
if (isset($_POST['form1'])) {
	// $tolevel = $_POST['tcat_id'];
	// $midlevel = $_POST['mcat_id'];
	// $endlevel = $_POST['ecat_id'];
	// $p_name = $_POST['p_name'];
	...
	if ($valid == 1) {
		...
		if (isset($_FILES['photo']["name"]) && isset($_FILES['photo']["tmp_name"])) {
			...
			for ($i = 0; $i < count($photo); $i++) {
				// Check if photo_temp array has a valid value to avoid undefined index
				if (!isset($photo_temp[$i]) || !is_uploaded_file($photo_temp[$i])) {
					continue; // Skip invalid files
				}

				$my_ext1 = pathinfo($photo[$i], PATHINFO_EXTENSION);
				// Validate allowed file extensions
				if (in_array(strtolower($my_ext1), ['jpg', 'png', 'jpeg', 'gif'])) {
					$final_name1[$m] = $z . '.' . $my_ext1;

					// Attempt to move the uploaded file and check for errors
					if (move_uploaded_file($photo_temp[$i], "../assets/uploads/product_photos/" . $final_name1[$m])) {
						$m++;
						$z++;
					} else {
						// Handle the error (e.g., permission issue or invalid destination)
						echo "Error uploading file: {$photo[$i]}";
					}
				}
			}
